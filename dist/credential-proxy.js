...
function logTokenUsage(model, promptTokens, completionTokens) {
    try {
        const db = getUsageDb();
        db.prepare('INSERT INTO groq_usage (model, prompt_tokens, completion_tokens, total_tokens) VALUES (?, ?, ?, ?)').run(model, promptTokens, completionTokens, promptTokens + completionTokens);
    }
    catch (err) {
...
export function startCredentialProxy(port, host = '127.0.0.1') {
    ...
    const authMode = (secrets.ANTHROPIC_API_KEY || (isNonAnthropicEndpoint && secrets.ANTHROPIC_AUTH_TOKEN))
        ? 'api-key'
        : 'oauth';
    ...
    return new Promise((resolve, reject) => {
        const server = createServer((req, res) => {
            ...
            req.on('end', () => {
                ...
                const isNvidiaBase = baseUrl.includes('integrate.api.nvidia.com') ||
                            ...
                            if (Array.isArray(parsed.messages)) {
                                ...
                                for (const msg of parsed.messages) {
                                    ...
                                    if (msg.role === 'assistant') {
                                        ...
                                        const converted = { role: 'assistant', content: textParts || null };
                    ...
                    if (isNvidiaResp && upRes.statusCode === 200) {
                        ...
                        upRes.on('end', () => {
                            try {
                                ...
                                // Log token usage
                                if (openaiResp.usage) {
                                    logTokenUsage(openaiResp.model || secrets.ANTHROPIC_DEFAULT_SONNET_MODEL || 'unknown', openaiResp.usage.prompt_tokens || 0, openaiResp.usage.completion_tokens || 0);
                                }
