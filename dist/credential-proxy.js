...
function logTokenUsage(model, promptTokens, completionTokens) {
    try {
        const db = getUsageDb();
        db.prepare('INSERT INTO llm_usage (model, prompt_tokens, completion_tokens, total_tokens) VALUES (?, ?, ?, ?)').run(model, promptTokens, completionTokens, promptTokens + completionTokens);
    }
    catch (err) {
