...
function buildVolumeMounts(
  ...
  fs.mkdirSync(path.join(groupIpcDir, 'subagent-requests'), { recursive: true });
  fs.mkdirSync(path.join(groupIpcDir, 'subagent-results'), { recursive: true });
  mounts.push({
...
function buildContainerArgs(
  ...
  // Route API traffic: always use credential proxy so the proxy can rewrite model names
  // and convert formats for non-Anthropic endpoints (NVIDIA, OpenRouter, etc.)
  // The proxy reads ANTHROPIC_BASE_URL from .env and forwards accordingly.
  ...
  // Proxy handles: auth injection, model rewriting (claude-* → OPENROUTER_MODEL), format conversion
  args.push(
    '-e',
  ...
  // Pass model names so SDK uses claude-* names (proxy rewrites them to actual model)
