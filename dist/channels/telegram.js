        ...
        const req = http.request({
            ...
            res.on('end', () => {
                ...
                if (res.statusCode === 200) {
                    try {
                        resolve(JSON.parse(rawBody).text.trim() || null);
                    }
                    catch {
                    ...
                    logger.warn({ status: res.statusCode, body: rawBody.slice(0, 200) }, 'Whisper transcription error');
                    resolve(null);
                }
            });
        });
...
` +
                `@Andy напиши письмо...
` +
                `@Andy переведи текст...`, { parse_mode: 'Markdown' });
        });
        ...
        this.bot.on('message:voice', async (ctx) => {
            ...
            if (fileId) {
                try {
                    ...
                    logger.warn({ err: e.message }, 'Voice transcription failed, using placeholder');
                }
            }
        ...
        this.bot.on('message:audio', async (ctx) => {
            ...
            if (fileId) {
                try {
                    ...
                    logger.warn({ err: e.message }, 'Audio transcription failed, using placeholder');
                }
            }
