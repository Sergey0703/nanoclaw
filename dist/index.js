...
import { ASSISTANT_NAME, CREDENTIAL_PROXY_PORT, DATA_DIR, IDLE_TIMEOUT, POLL_INTERVAL, TIMEZONE, TRIGGER_PATTERN, } from './config.js';
...
function getSessionJsonlSize(groupFolder, sessionId) {
    if (!sessionId)
        return 0;
...
async function compactSession(group, sessionId, chatJid) {
    ...
    try {
        ...
        }, (proc, containerName) => queue.registerProcess(chatJid, proc, containerName, group.folder), async (output) => {
            if (output.result)
                summary = output.result;
        });
    }
    catch (err) {
        logger.warn({ err }, 'Failed to generate session summary');
    }
    return summary;
}
